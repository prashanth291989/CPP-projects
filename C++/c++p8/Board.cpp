#include "Board.h"
#include "cluster.h"
const char* Board::filename="puz1.txt";
//------------------------Parameterized constructor of Board -------------------//
Board::Board(ifstream &inputpuzzle,int n): n(n){
    char input;int temp;
    Square *sq[9];
    unmarked=81;
    clus=new cluster*[n];
    input=inputpuzzle.get();
    if(!inputpuzzle) throw FileNotOpen(Board::filename);
        for( int j=1; j<10; j++){
            for( int k=1; k<10; k++){
                input=inputpuzzle.get();
                if(isdigit(input)|| input=='-'){
                    ...
                    throw MalformedFile(Board::filename);
                }
            }
...
void Board::MarkChar( int row, int col,char value ){
     bd[sub(row,col)].mark(value,row,value);
}
...
//-----------------------Sub script function for  Board ---------------------//
const Square& Board::sub(short int j,short int k )const{
    return bd[(j-1)*9+(k-1)];
}
