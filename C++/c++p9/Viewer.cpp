...
Viewer::Viewer(int nr, int nc, CanView* model) :
		numRows(nr), numCols(nc), sqInBox(3), sqWid(9), sqHig(4), colSpan(
				sqWid + 1), rowSpan(sqHig + 1), width(colSpan * numCols + 1), height(
				rowSpan * numRows + 1), cv(new Canvas(height, width)), model(model) {
	drawGrid();
}
...
void Viewer::show(ostream& out) {
	for (int row = 1; row <= numRows; ++row)
		for (int col = 1; col <= numCols; ++col) {
			int sr0 = (row-1) * rowSpan + 1;	// disp coords of UL square corner
